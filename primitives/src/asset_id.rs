...
impl AssetId {
    pub fn new(asset_id: String) -> Option<Self> {
...
mod tests {
    ...
    fn test_new_asset_id_with_coin() {
        let asset_id = AssetId::new("ethereum".to_owned()).unwrap();
    ...
    fn test_new_asset_id_with_coin_and_token() {
        let asset_id = AssetId::new("ethereum_0x1234567890abcdef".to_owned()).unwrap();
        assert_eq!(asset_id.chain, Chain::Ethereum);
