...
pub async fn jobs(settings: Settings, shutdown_rx: ShutdownReceiver) -> Result<Vec<JoinHandle<()>>, Box<dyn Error + Send + Sync>> {
    ...
    let update_existing_assets = tokio::spawn(run_job("Update existing prices assets", config.get_duration(ConfigKey::AssetsTimerUpdateExisting)?, shutdown_rx.clone(), {
    ...
    let update_all_assets = tokio::spawn(run_job("Update all prices assets", config.get_duration(ConfigKey::AssetsTimerUpdateAll)?, shutdown_rx.clone(), {
        let (coingecko_client, database, cacher_client) = (coingecko_client.clone(), database.clone(), cacher_client.clone());
        move || {
            let asset_updater = AssetUpdater::new(coingecko_client.clone(), database.clone(), cacher_client.clone());
    ...
    let update_native_prices_assets = tokio::spawn(run_job("Update native prices assets", config.get_duration(ConfigKey::AssetsTimerUpdateNative)?, shutdown_rx.clone(), {
        let (coingecko_client, database, cacher_client) = (coingecko_client.clone(), database.clone(), cacher_client.clone());
        move || {
            let asset_updater = AssetUpdater::new(coingecko_client.clone(), database.clone(), cacher_client.clone());
    ...
    let update_tranding_assets = tokio::spawn(run_job("Update CoinGecko Trending assets", config.get_duration(ConfigKey::AssetsTimerUpdateTrending)?, shutdown_rx.clone(), {
        let (coingecko_client, database, cacher_client) = (coingecko_client.clone(), database.clone(), cacher_client.clone());
        move || {
            let asset_updater = AssetUpdater::new(coingecko_client.clone(), database.clone(), cacher_client.clone());
    ...
    let update_recently_added_assets = tokio::spawn(run_job("Update CoinGecko recently added assets", config.get_duration(ConfigKey::AssetsTimerUpdateRecentlyAdded)?, shutdown_rx.clone(), {
        let (coingecko_client, database, cacher_client) = (coingecko_client.clone(), database.clone(), cacher_client.clone());
        move || {
            let asset_updater = AssetUpdater::new(coingecko_client.clone(), database.clone(), cacher_client.clone());
    ...
    let update_suspicious_assets = tokio::spawn(run_job("Update suspicious asset ranks", config.get_duration(ConfigKey::AssetsTimerUpdateSuspicious)?, shutdown_rx.clone(), {
    ...
    let update_staking_apy = tokio::spawn(run_job("Update staking APY", config.get_duration(ConfigKey::AssetsTimerUpdateStakingApy)?, shutdown_rx.clone(), {
    ...
    let update_perpetuals = tokio::spawn(run_job("Update perpetuals", config.get_duration(ConfigKey::AssetsTimerUpdatePerpetuals)?, shutdown_rx.clone(), {
        let settings = settings.clone();
        let database = database.clone();
        move || {
    ...
    let update_usage_ranks = tokio::spawn(run_job("Update usage ranks", config.get_duration(ConfigKey::AssetsTimerUpdateUsageRank)?, shutdown_rx.clone(), {
        let database = database.clone();
        move || {
