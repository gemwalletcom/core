...
pub async fn jobs(settings: Settings, shutdown_rx: ShutdownReceiver) -> Result<Vec<JoinHandle<()>>, Box<dyn Error + Send + Sync>> {
    ...
    let update_validators = tokio::spawn(run_job("Update chain validators", config.get_duration(ConfigKey::ScanTimerUpdateValidators)?, shutdown_rx.clone(), {
    ...
    let update_validators_static_assets = tokio::spawn(run_job("Update validators from static assets", config.get_duration(ConfigKey::ScanTimerUpdateValidatorsStatic)?, shutdown_rx, {
        let settings = settings.clone();
        let database = database.clone();
        move || {
            let validator_scanner = ValidatorScanner::new(
