...
pub async fn jobs(settings: Settings) -> Result<Vec<Pin<Box<dyn Future<Output = ()> + Send>>>, Box<dyn Error + Send + Sync>> {
    ...
    let pricer_timer = database.client()?.config().get_config_value_duration(ConfigKey::PricerTimer)?;
    let pricer_outdated = database.client()?.config().get_config_value_duration(ConfigKey::PricerOutdated)?;
