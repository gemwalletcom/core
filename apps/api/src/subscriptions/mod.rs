...
pub async fn get_subscriptions(device_id: DeviceIdParam, client: &State<Mutex<SubscriptionsClient>>) -> Result<ApiResponse<Vec<Subscription>>, ApiError> {
    Ok(client.lock().await.get_subscriptions_by_device_id(&device_id.0).await?.into())
}
