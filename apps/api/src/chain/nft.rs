...
pub async fn get_nft_asset(asset_id: NftAssetIdParam, client: &State<NFTClient>) -> Result<ApiResponse<NFTAsset>, ApiError> {
    Ok(client.get_nft_asset(asset_id.0).await?.into())
}

#[get("/chain/nft/collections/<collection_id>")]
pub async fn get_nft_collection(collection_id: NftCollectionIdParam, client: &State<NFTClient>) -> Result<ApiResponse<NFTCollection>, ApiError> {
    Ok(client.get_nft_collection(collection_id.0).await?.into())
}

#[get("/chain/address/<chain>/<address>/nfts")]
pub async fn get_nfts(chain: ChainParam, address: AddressParam, client: &State<NFTClient>) -> Result<ApiResponse<Vec<NFTData>>, ApiError> {
