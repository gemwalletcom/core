...
use primitives::{NewSupportDevice, SupportDevice};
use rocket::{State, get, post, serde::json::Json, tokio::sync::Mutex};

#[post("/support/add_device", format = "json", data = "<request>")]
pub async fn add_device(request: Json<NewSupportDevice>, client: &State<Mutex<SupportClient>>) -> Result<ApiResponse<SupportDevice>, ApiError> {
