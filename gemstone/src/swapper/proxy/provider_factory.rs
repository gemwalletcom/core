...
pub fn new_near_intents() -> ProxyProvider {
    ProxyProvider {
        provider: SwapperProviderType::new(SwapperProvider::NearIntents),
        url: format!("{}/{}", PROVIDER_API_URL, "near_intents"),
        assets: vec![
            SwapperChainAsset::All(Chain::Near),
            SwapperChainAsset::Assets(
                Chain::Ethereum,
                vec![
                    ETHEREUM_USDT.id.clone(),
            ...
            SwapperChainAsset::All(Chain::Bitcoin),
            SwapperChainAsset::Assets(Chain::Solana, vec![SOLANA_USDC.id.clone(), SOLANA_USDT.id.clone(), SOLANA_WBTC.id.clone()]),
            SwapperChainAsset::Assets(Chain::Sui, vec![SUI_USDC.id.clone(), SUI_SBUSDT.id.clone()]),
            SwapperChainAsset::Assets(Chain::Arbitrum, vec![ARBITRUM_USDC.id.clone(), ARBITRUM_USDT.id.clone()]),
            SwapperChainAsset::Assets(Chain::Base, vec![BASE_USDC.id.clone()]),
            SwapperChainAsset::Assets(Chain::Optimism, vec![OPTIMISM_USDC.id.clone(), OPTIMISM_USDT.id.clone()]),
            SwapperChainAsset::Assets(Chain::AvalancheC, vec![AVALANCHE_USDT.id.clone(), AVALANCHE_USDC.id.clone()]),
            SwapperChainAsset::Assets(Chain::SmartChain, vec![SMARTCHAIN_USDT.id.clone(), SMARTCHAIN_USDC.id.clone()]),
            SwapperChainAsset::Assets(Chain::Polygon, vec![POLYGON_USDC.id.clone(), POLYGON_USDT.id.clone()]),
            SwapperChainAsset::All(Chain::Ton),
