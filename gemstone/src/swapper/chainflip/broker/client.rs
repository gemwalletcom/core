use super::model::{ChainflipAsset, DcaParameters, DepositAddressResponse, RefundParameters, VaultSwapExtraParams, VaultSwapResponse};
use crate::{
    network::{jsonrpc::jsonrpc_call_with_endpoint, AlienProvider},
...
impl BrokerClient {
    ...
    pub async fn get_deposit_address(
        ...
        let url = format!("{}/rpc/{}", CHAINFLIP_BROKER_URL, CHAINFLIP_BROKER_KEY);
        let params = json!([
            src_asset,
            dst_asset,
            dst_address,
        ...
        jsonrpc_call_with_endpoint(self.provider.clone(), &url, "broker_request_swap_deposit_address", params)
            .await
    ...
    pub async fn encode_vault_swap(
        ...
        jsonrpc_call_with_endpoint(self.provider.clone(), &url, "broker_request_swap_parameter_encoding", params)
            .await
            .map_err(SwapperError::from)
    }
}
