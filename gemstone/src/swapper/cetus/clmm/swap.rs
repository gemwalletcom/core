...
/// Represents the result of a swap computation for a single tick range
pub struct SingleTickSwapResult {
...
pub fn compute_amount_a_delta(liquidity: &BigInt, sqrt_price_next: &BigInt, sqrt_price_current: &BigInt, round_up: bool) -> BigInt {
    ...
    if sqrt_price_next > sqrt_price_current {
        ...
        if round_up {
            delta_x = div_round_up(&numerator, &denominator);
        } else {
...
pub fn compute_amount_b_delta(liquidity: &BigInt, sqrt_price_next: &BigInt, sqrt_price_current: &BigInt, round_up: bool) -> BigInt {
    ...
    if sqrt_price_next > sqrt_price_current {
        ...
        if round_up {
            delta_y = div_round_up(&(liquidity * delta), &BigInt::from(2).pow(64));
        } else {
...
pub fn compute_swap_step_single_tick(
...
pub fn estimate_liquidity_for_coin_a(sqrt_price_x: &BigInt, sqrt_price_y: &BigInt, coin_amount: &BigInt) -> BigInt {
    ...
    let num = from_x64_bn(&(coin_amount * upper_sqrt_price_x64 * lower_sqrt_price_x64));
...
pub fn update_fee_rate(clmm: &mut ClmmPoolData, fee_amount: &BigInt, ref_rate: i64, protocol_fee_rate: i64, is_coin_a: bool) -> Result<BigInt, ErrorCode> {
    let protocol_fee = check_mul_div_ceil(fee_amount, &BigInt::from(protocol_fee_rate), &FEE_RATE_DENOMINATOR, 64)?;

    let ref_fee = if ref_rate == 0 {
        ZERO.clone()
    } else {
        check_mul_div_floor(fee_amount, &BigInt::from(ref_rate), &FEE_RATE_DENOMINATOR, 64)?
    };
...
pub fn get_coin_amount_from_liquidity(
    ...
    if cur_sqrt_price < lower_sqrt_price {
        ...
        let term1 = from_x64_decimal(&liq);
...
pub fn est_liquidity_and_coin_amount_from_one_amounts(
    ...
    let coin_amounts = get_coin_amount_from_liquidity(&liquidity, cur_sqrt_price, &lower_sqrt_price, &upper_sqrt_price, round_up);
...
pub fn est_coin_amounts_from_total_amount(
    ...
    let ratio = calculate_deposit_ratio_fix_token_a(lower_tick, upper_tick, cur_sqrt_price).unwrap();
...
pub fn calculate_deposit_ratio_fix_token_a(lower_tick: i32, upper_tick: i32, cur_sqrt_price: &BigInt) -> Result<(BigDecimal, BigDecimal), ErrorCode> {
    ...
    let liquidity_input =
        est_liquidity_and_coin_amount_from_one_amounts(lower_tick, upper_tick, &coin_amount_a, true, true, BigDecimal::zero(), cur_sqrt_price)?;
...
mod tests {
    ...
    fn test_get_coin_amount_from_liquidity() {
        ...
        // Price is in range
        let in_range = get_coin_amount_from_liquidity(&liquidity, &current_sqrt_price, &lower_sqrt_price, &upper_sqrt_price, false);
        ...
        let below_range = get_coin_amount_from_liquidity(
            &liquidity,
        ...
        let above_range = get_coin_amount_from_liquidity(
            &liquidity,
        ...
        // Test rounding
        let round_up = get_coin_amount_from_liquidity(&liquidity, &current_sqrt_price, &lower_sqrt_price, &upper_sqrt_price, true);

        let round_down = get_coin_amount_from_liquidity(&liquidity, &current_sqrt_price, &lower_sqrt_price, &upper_sqrt_price, false);
    ...
    #[test]
    fn test_compute_swap_step_single_tick_a_to_b_exact_input() {
        ...
        let result = compute_swap_step_single_tick(
            &sqrt_price_current,
            &sqrt_price_target,
            &liquidity,
            &amount_remaining,
    ...
    #[test]
    fn test_compute_swap_step_single_tick_a_to_b_exact_output() {
    ...
    #[test]
    fn test_compute_swap_step_single_tick_b_to_a_exact_input() {
    ...
    #[test]
    fn test_compute_swap_step_single_tick_b_to_a_exact_output() {
