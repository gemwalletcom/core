...
impl GemSwapper {
    ...
    pub fn new(rpc_provider: Arc<dyn AlienProvider>) -> Self {
        let swappers: Vec<Box<dyn Swapper>> = vec![
            Box::new(uniswap::default::new_uniswap_v3(rpc_provider.clone())),
            Box::new(uniswap::default::new_uniswap_v4(rpc_provider.clone())),
            Box::new(uniswap::default::new_pancakeswap(rpc_provider.clone())),
            ...
            Box::new(uniswap::default::new_oku(rpc_provider.clone())),
            Box::new(uniswap::default::new_wagmi(rpc_provider.clone())),
            pancakeswap_aptos::default::boxed_pancakeswap_aptos(rpc_provider.clone()),
            Box::new(provider_factory::new_stonfi_v2(rpc_provider.clone())),
            Box::new(provider_factory::new_mayan(rpc_provider.clone())),
            chainflip::default::boxed_chainflip_provider(rpc_provider.clone()),
            cetus::default::boxed_cetus(rpc_provider.clone()),
            Box::new(provider_factory::new_cetus_aggregator(rpc_provider.clone())),
            Box::new(provider_factory::new_relay(rpc_provider.clone())),
            Box::new(uniswap::default::new_aerodrome(rpc_provider.clone())),
        ];
...
mod tests {
    ...
    fn test_adjust_request_for_stable_swap_updates_referral_fees() {
        let referral_fees = SwapReferralFees {
            evm: SwapReferralFee {
                address: "0xevm".into(),
