...
use primitives::{
    ...
    TransactionLoadMetadata, TransactionMetadata, TransactionPerpetualMetadata, TransactionStateRequest, TransactionUpdate, TransferDataExtra, TransferDataOutputType,
    ...
    fn from(value: GemTransactionLoadFee) -> Self {
        TransactionFee {
            ...
            options: value
                ...
                .into_iter()
                .map(|(key, value)| {
                    (key, value.parse().unwrap_or_default())
                })
                .collect(),
        }
    }
...
impl From<GemTransactionInputType> for TransactionInputType {
    fn from(value: GemTransactionInputType) -> Self {
        use primitives::{swap::SwapData, swap::ApprovalData};
        match value {
            ...
            GemTransactionInputType::Stake { asset, stake_type } => {
                TransactionInputType::Stake(asset.into(), stake_type.into())
            },
            GemTransactionInputType::TokenApprove { asset, approval_data } => {
            ...
            GemTransactionInputType::Generic { asset, metadata, extra } => {
            ...
            GemTransactionInputType::Perpetual { asset, perpetual_type } => {
                TransactionInputType::Perpetual(asset.into(), perpetual_type.into())
            },
        }
    }
}
...
impl From<GemTransactionLoadMetadata> for TransactionLoadMetadata {
    fn from(value: GemTransactionLoadMetadata) -> Self {
        match value {
            ...
            GemTransactionLoadMetadata::Solana { sender_token_address, recipient_token_address, token_program, block_hash } => {
                TransactionLoadMetadata::Solana {
                    sender_token_address,
                    recipient_token_address,
                    token_program: token_program.map(|t| t.into()),
                    block_hash
                }
            },
            GemTransactionLoadMetadata::Ton { sender_token_address, recipient_token_address, sequence } => {
                TransactionLoadMetadata::Ton { sender_token_address, recipient_token_address, sequence }
            },
            GemTransactionLoadMetadata::Cosmos { gas_multiplier: _ } => {
            ...
            GemTransactionLoadMetadata::Sui { coins: _ } => {
                TransactionLoadMetadata::Sui { message_bytes: String::new() }
            },
            GemTransactionLoadMetadata::Bitcoin { utxos } => {
                TransactionLoadMetadata::Bitcoin { utxos: utxos.into_iter().map(|u| u.into()).collect() }
            },
            GemTransactionLoadMetadata::Stake { data: _ } => {
...
impl From<TransactionLoadMetadata> for GemTransactionLoadMetadata {
    fn from(value: TransactionLoadMetadata) -> Self {
        match value {
            ...
            TransactionLoadMetadata::Solana { sender_token_address, recipient_token_address, token_program, block_hash } => {
            ...
            TransactionLoadMetadata::Cosmos { account_number: _, sequence: _, chain_id: _ } => {
                GemTransactionLoadMetadata::Cosmos { gas_multiplier: 1.0 }
            },
            TransactionLoadMetadata::Sui { message_bytes: _ } => {
                GemTransactionLoadMetadata::Sui { coins: vec![] }
            },
            TransactionLoadMetadata::Bitcoin { utxos } => {
                GemTransactionLoadMetadata::Bitcoin { utxos: utxos.into_iter().map(|u| u.into()).collect() }
            },
            _ => GemTransactionLoadMetadata::None,
        }
    }
}
