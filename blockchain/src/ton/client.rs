...
use super::model::{Chainhead, JSONResult, Shards, ShortTransactions, Transaction};
...
impl TonClient {
    ...
    pub async fn get_transactions(
        ...
        shard: i64,
        sequence: i64,
    ) -> Result<ShortTransactions, Box<dyn Error + Send + Sync>> {
        let url = format!(
            "{}/api/v2/getBlockTransactions?workchain={}&shard={}&seqno={}&count=50",
            self.url, workchain, shard, sequence
        );
        let response = self
            .client
            .get(url)
    ...
    pub async fn get_transaction(
        ...
        let url = Url::parse_with_params(
            format!("{}/api/v2/getTransactions", self.url).as_str(),
            &[("address", address), ("lt", lt), ("limit", "1".to_string())],
        )
        .unwrap();
