...
fn main() {
    ...
    let ignored_files = ["lib.rs", "mod.rs", "client.rs", "model.rs", "address.rs", "address_formatter.rs", "big_int_hex.rs"];
    ...
    for folder in folders {
        ...
        for path in paths {
            ...
            match platform_str {
                "ios" => {
                    println!("Generate file for iOS: {}, output: {}", input_path, ios_output_path);
                    generate_files(LANGUAGE_SWIFT, input_path.as_str(), ios_output_path.as_str(), "");
                },
                "android" => {
                    println!("Generate file for Android: {}, output: {}", input_path, android_output_path);
                    generate_files(LANGUAGE_KOTLIN, input_path.as_str(), android_output_path.as_str(), android_package_name.as_str());
                },
                _ => {
