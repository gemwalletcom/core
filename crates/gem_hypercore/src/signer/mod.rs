...
use primitives::{
    HyperliquidOrder, NumberIncrementer, PerpetualDirection, PerpetualType, SignerError, TransactionInputType, TransactionLoadInput, TransactionLoadMetadata,
    stake_type::StakeType, swap::SwapData,
};
use serde::Serialize;
...
impl HypercoreSigner {
    ...
    pub fn sign_transfer(&self, input: &TransactionLoadInput, private_key: &[u8]) -> SignerResult<String> {
    ...
    pub fn sign_token_transfer(&self, input: &TransactionLoadInput, private_key: &[u8]) -> SignerResult<String> {
        let asset = input.input_type.get_asset();
        let amount = BigNumberFormatter::value(&input.value, asset.decimals).map_err(|err| SignerError::InvalidInput(err.to_string()))?;
    ...
    pub fn sign_swap(&self, input: &TransactionLoadInput, private_key: &[u8]) -> SignerResult<Vec<String>> {
        let swap_data = extract_swap_data(&input.input_type)?;
        let signature = sign_typed_action(&swap_data.data.data, private_key)?;
        Ok(vec![signature])
    }

    pub fn sign_stake(&self, input: &TransactionLoadInput, private_key: &[u8]) -> SignerResult<Vec<String>> {
        let stake_type = extract_stake_type(&input.input_type)?;
        let mut nonce_incrementer = NumberIncrementer::new(timestamp_ms());
        ...
        match stake_type {
            StakeType::Stake(validator) => {
                ...
                let deposit_action = sign_c_deposit(deposit_request, private_key)?;

                let delegate_request = TokenDelegate::new(validator.id.clone(), wei, false, nonce_incrementer.next());
                let delegate_action = sign_token_delegate(delegate_request, private_key)?;
            ...
            StakeType::Unstake(delegation) => {
                ...
                let undelegate_action = sign_token_delegate(undelegate_request, private_key)?;

                let withdraw_request = CWithdraw::new(wei, nonce_incrementer.next());
                let withdraw_action = sign_c_withdraw(withdraw_request, private_key)?;
    ...
    pub fn sign_nft_transfer(&self, _input: &TransactionLoadInput, _private_key: &[u8]) -> SignerResult<String> {
        Err(SignerError::UnsupportedOperation("NFT transfer not supported".to_string()))
    }

    pub fn sign_token_approval(&self, _input: &TransactionLoadInput, _private_key: &[u8]) -> SignerResult<String> {
        Err(SignerError::UnsupportedOperation("Token approval not supported".to_string()))
    }

    pub fn sign_account_action(&self, _input: &TransactionLoadInput, _private_key: &[u8]) -> SignerResult<String> {
        Err(SignerError::UnsupportedOperation("Account action not supported".to_string()))
    }

    pub fn sign_perpetual(&self, input: &TransactionLoadInput, private_key: &[u8]) -> SignerResult<Vec<String>> {
        ...
        let mut timestamp_incrementer = NumberIncrementer::new(timestamp_ms());
        let mut transactions = Vec::new();

        if order.approve_referral_required {
            transactions.push(self.sign_set_referrer(private_key, REFERRAL_CODE, timestamp_incrementer.next())?);
        }
        ...
        transactions.push(self.sign_market_message(perpetual_type, agent_key.as_slice(), builder, timestamp_incrementer.next())?);

        Ok(transactions)
    }

    pub fn sign_withdrawal(&self, _input: &TransactionLoadInput, _private_key: &[u8]) -> SignerResult<String> {
        Err(SignerError::UnsupportedOperation("Withdrawal not supported".to_string()))
    }

    pub fn sign_data(&self, _input: &TransactionLoadInput, _private_key: &[u8]) -> SignerResult<String> {
        Err(SignerError::UnsupportedOperation("Data signing not supported".to_string()))
    }

    pub fn sign_typed_action(&self, typed_data_json: &str, private_key: &[u8]) -> SignerResult<String> {
        sign_typed_action(typed_data_json, private_key)
    }
    ...
    fn sign_approve_agent(&self, agent_address: &str, private_key: &[u8], timestamp: u64) -> SignerResult<String> {
        ...
        sign_approve_agent(agent, private_key)
    }
    ...
    fn sign_approve_builder_address(&self, agent_key: &[u8], builder_address: &str, rate_bps: u32, timestamp: u64) -> SignerResult<String> {
        ...
        sign_approve_builder_fee(request, agent_key)
    }
    ...
    fn sign_set_referrer(&self, agent_key: &[u8], code: &str, timestamp: u64) -> SignerResult<String> {
        ...
        sign_set_referrer(referer, timestamp, agent_key)
    }
    ...
    fn sign_spot_send(&self, amount: &str, destination: &str, token: &str, private_key: &[u8]) -> SignerResult<String> {
        ...
        sign_spot_send(spot_send, private_key)
    }
    ...
    fn sign_market_message(&self, perpetual_type: &PerpetualType, agent_key: &[u8], builder: Option<Builder>, timestamp: u64) -> SignerResult<String> {
        let (data, is_open) = match perpetual_type {
            PerpetualType::Open(data) => (data, true),
        ...
        sign_place_order(order, timestamp, agent_key)
    }
}

pub fn sign_typed_action(typed_data_json: &str, private_key: &[u8]) -> SignerResult<String> {
    ...
    let timestamp =
        serde_json::from_value::<TimestampField>(message.clone()).map_err(|err| SignerError::InvalidInput(format!("Failed to parse time or nonce: {err}")))?;
    ...
    sign_action(typed_data_json, &action, timestamp.value, private_key)
}

pub fn sign_spot_send(spot_send: SpotSend, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = spot_send.time;
    sign_serialized_action(spot_send, timestamp, private_key, send_spot_token_to_address_typed_data, "spot send")
}

pub fn sign_withdrawal_request(request: WithdrawalRequest, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = request.time;
    sign_serialized_action(request, timestamp, private_key, withdrawal_request_typed_data, "withdrawal request")
}

pub fn sign_approve_agent(agent: ApproveAgent, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = agent.nonce;
    sign_serialized_action(agent, timestamp, private_key, approve_agent_typed_data, "approve agent")
}

pub fn sign_approve_builder_fee(fee: ApproveBuilderFee, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = fee.nonce;
    sign_serialized_action(fee, timestamp, private_key, approve_builder_fee_typed_data, "approve builder fee")
}

pub fn sign_set_referrer(referrer: SetReferrer, nonce: u64, private_key: &[u8]) -> SignerResult<String> {
    sign_serialized_action(referrer, nonce, private_key, |value| set_referrer_typed_data(value, nonce), "set referrer")
}

pub fn sign_c_deposit(deposit: CDeposit, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = deposit.nonce;
    sign_serialized_action(deposit, timestamp, private_key, c_deposit_typed_data, "c deposit")
}

pub fn sign_c_withdraw(withdraw: CWithdraw, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = withdraw.nonce;
    sign_serialized_action(withdraw, timestamp, private_key, c_withdraw_typed_data, "c withdraw")
}

pub fn sign_token_delegate(delegate: TokenDelegate, private_key: &[u8]) -> SignerResult<String> {
    let timestamp = delegate.nonce;
    sign_serialized_action(delegate, timestamp, private_key, token_delegate_typed_data, "token delegate")
}

pub fn sign_place_order(order: PlaceOrder, nonce: u64, private_key: &[u8]) -> SignerResult<String> {
    sign_serialized_action(order, nonce, private_key, |value| place_order_typed_data(value, nonce), "place order")
}

fn sign_action(typed_data: &str, action: &str, timestamp: u64, private_key: &[u8]) -> SignerResult<String> {
    ...
    build_signed_request(signature, action, timestamp)
}

fn sign_serialized_action<T, F>(value: T, timestamp: u64, private_key: &[u8], typed_data_fn: F, action_name: &'static str) -> SignerResult<String>
where
    T: Serialize,
    F: FnOnce(T) -> String,
{
...
fn build_signed_request(signature: String, action: &str, timestamp: u64) -> SignerResult<String> {
