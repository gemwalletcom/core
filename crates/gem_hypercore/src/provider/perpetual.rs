...
impl<C: Client> HyperCoreClient<C> {
    ...
    async fn fetch_positions_for_dex(&self, address: String, dex: Option<String>) -> Result<PerpetualPositionsSummary, Box<dyn Error + Sync + Send>> {
        let (positions, orders) = match dex.as_deref() {
            Some(dex) => try_join!(self.get_clearinghouse_state_with_dex(&address, dex), self.get_open_orders_with_dex(&address, dex))?,
            None => try_join!(self.get_clearinghouse_state(&address), self.get_open_orders(&address))?,
        };
