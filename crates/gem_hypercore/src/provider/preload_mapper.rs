...
pub(crate) async fn get_approvals_and_credentials(
...
) -> Result<(bool, bool, bool, UserFeeRates, String, String), Box<dyn Error + Send + Sync>> {
    let ((agent_required, agent_address, agent_private_key), referral_required, builder_required, fee_rates) = futures::try_join!(
        ...
        cache.get_user_fee_rates(sender_address, get_user_fees),
    )?;

    Ok((agent_required, referral_required, builder_required, fee_rates, agent_address, agent_private_key))
}
