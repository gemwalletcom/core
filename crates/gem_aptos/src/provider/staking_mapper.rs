...
use primitives::{Chain, EarnPositionData, EarnPositionState, EarnProvider, EarnProviderType};

use crate::models::{DelegationPoolStake, StakingConfig, ValidatorInfo, ValidatorSet};

pub fn map_validators(validator_set: ValidatorSet, apy: f64, pool_address: &str, commission: f64) -> Vec<EarnProvider> {
    validator_set
        .active_validators
        .iter()
...
pub fn map_validator(validator: &ValidatorInfo, apy: f64, commission: f64, is_active: bool) -> EarnProvider {
    EarnProvider {
        chain: Chain::Aptos,
...
fn map_delegation(asset_id: &primitives::AssetId, state: EarnPositionState, balance: BigUint, provider_id: &str, completion_date: Option<DateTime<Utc>>) -> EarnPositionData {
    EarnPositionData {
        ...
        completion_date,
        position_id: format!("{}_{}", state.as_ref().to_lowercase(), provider_id),
        provider_id: provider_id.to_string(),
    }
}

pub fn map_delegations(stakes: Vec<(String, DelegationPoolStake)>, lockup_secs: u64) -> Vec<EarnPositionData> {
    ...
    stakes
        ...
        .flat_map(|(pool_address, stake)| {
            ...
            if stake.active > BigUint::from(0u32) {
                delegations.push(map_delegation(&asset_id, EarnPositionState::Active, stake.active, &pool_address, None));
            }
            ...
            if stake.inactive > BigUint::from(0u32) {
                delegations.push(map_delegation(&asset_id, EarnPositionState::AwaitingWithdrawal, stake.inactive, &pool_address, None));
            }
...
mod tests {
    ...
    fn test_map_delegations_active() {
        ...
        assert_eq!(delegations[0].state, EarnPositionState::Active);
    ...
    fn test_map_delegations_pending_inactive() {
        ...
        assert_eq!(delegations.len(), 1);
        assert_eq!(delegations[0].state, EarnPositionState::Deactivating);
    ...
    fn test_map_delegations_inactive() {
        ...
        assert_eq!(delegations.len(), 1);
        assert_eq!(delegations[0].state, EarnPositionState::AwaitingWithdrawal);
