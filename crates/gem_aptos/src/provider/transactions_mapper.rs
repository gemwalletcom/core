...
pub fn map_transaction_broadcast(
    ...
    response.hash.clone().ok_or_else(|| {
        "Transaction response missing hash".into()
    })
}
...
pub fn map_transaction_status(
    ...
    let mut update = TransactionUpdate::new(state);
...
mod tests {
    ...
    fn test_map_transaction_status_confirmed() {
        ...
        assert_eq!(result.changes, vec![TransactionChange::NetworkFee("100".to_string())]); // 100 * 1 = 100
    }
    ...
    fn test_map_transaction_status_failed() {
        ...
        assert_eq!(result.changes, vec![TransactionChange::NetworkFee("50".to_string())]); // 50 * 1 = 50
    }
}