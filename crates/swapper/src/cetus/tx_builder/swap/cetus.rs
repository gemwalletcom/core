use super::super::{
    constants::{FUNCTION_SWAP, MODULE_CETUS},
    error::tx_error,
    model::SwapStep,
};
use crate::{
    SwapperError,
    cetus::{
        constants::{CETUS_GLOBAL_CONFIG, CETUS_PARTNER},
        model::FlattenedPath,
    },
};
use gem_sui::{
...
pub(super) fn build_swap(txb: &mut TransactionBuilder, resolver: &ObjectResolver, flattened_path: &FlattenedPath, swap_context: Argument) -> Result<(), SwapperError> {
    ...
    let global_config = resolver.shared_object(txb, CETUS_GLOBAL_CONFIG, true).map_err(tx_error)?;
    ...
    move_call(
        ...
        vec![swap_context, global_config, pool, partner, direction, amount_in, clock],
    )
    .map_err(tx_error)?;
    Ok(())
}
