...
use primitives::{Chain, EarnProvider, EarnProviderType, StakeValidator};
...
pub fn map_staking_validators(witnesses: WitnessesList, apy: Option<f64>) -> Vec<EarnProvider> {
    let default_apy = apy.unwrap_or(0.0);
    let mut validators: Vec<EarnProvider> = witnesses
        .witnesses
        .into_iter()
        .filter_map(|witness| {
            Some(EarnProvider {
