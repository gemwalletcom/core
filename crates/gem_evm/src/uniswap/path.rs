...
pub fn get_base_pair(chain: &EVMChain) -> BasePair {
    let weth = chain.weth_contract().unwrap().to_string();
    ...
    BasePair {
        native: EthereumAddress::parse(&weth).unwrap(),
        stable: EthereumAddress::parse(stable).unwrap(),
    }
}
