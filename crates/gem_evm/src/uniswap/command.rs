...
pub fn encode_commands_with_deadline(commands: &[UniversalRouterCommand], deadline: U256) -> Vec<u8> {
...
pub fn encode_commands(commands: &[UniversalRouterCommand]) -> Vec<u8> {
    let commands_bytes: Vec<u8> = commands.iter().map(|command| command.raw_value()).collect();
    let inputs: Vec<Bytes> = commands.iter().map(|command| Bytes::from_iter(command.encode().iter())).collect();
    let call = Dispatcher::executeCall {
        commands: Bytes::from_iter(commands_bytes.iter()),
        inputs,
    };
    call.abi_encode()
...
mod tests {
    ...
    fn test_encode_eth_to_usdc() {
        ...
        let encoded = encode_commands_with_deadline(&commands, deadline);
