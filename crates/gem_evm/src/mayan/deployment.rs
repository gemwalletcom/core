...
pub fn get_swift_providers() -> HashMap<Chain, MayanSwiftDeployment> {
    let mayan_swift_evm = "0xC38e4e6A15593f908255214653d3D947CA1c2338";
    let mayan_swift_solana = "BLZRi6frs4X4DNLw56V4EXai1b6QVESN1BhHBTYM9VcY";
    HashMap::from([
        (
            Chain::Solana,
            MayanSwiftDeployment {
                address: mayan_swift_solana.to_string(),
        ...
        (
            Chain::Ethereum,
            MayanSwiftDeployment {
                address: mayan_swift_evm.to_string(),
        ...
        (
            ...
            MayanSwiftDeployment {
                address: mayan_swift_evm.to_string(),
                wormhole_id: WormholeId::Avalanche,
            },
        ),
    ])
}
...
mod tests {
    ...
    fn test_chain_ids() {
        ...
        assert_eq!(
            get_swift_deployment_by_chain(Chain::AvalancheC).map(|x| x.wormhole_id),
            Some(WormholeId::Avalanche)
        );
        assert_eq!(
