use primitives::{AssetPriceOnly, Chain};

use crate::PriceChainProvider;
use async_trait::async_trait;

use super::client::PythClient;
...
impl PriceChainProvider for PythProvider {
    async fn get_chain_prices(&self, chains: Vec<primitives::Chain>) -> Result<Vec<primitives::AssetPriceOnly>, Box<dyn std::error::Error + Send + Sync>> {
        let mut prices = Vec::new();
        for chain in chains {
            let price_account = self.price_account(chain);
            let price = self.pyth_client.get_price(price_account).await?;
            prices.push(AssetPriceOnly {
...
impl PythProvider {
    pub fn price_account(&self, chain: Chain) -> &str {
        match chain {
