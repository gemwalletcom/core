...
impl CellSlice {
    ...
    pub fn new_with_offset(cell: &Cell, offset: usize) -> Result<CellSlice, TonCellError> {
        CellSlice::new(&Arc::new(cell.clone()), offset, cell.bit_len, 0, cell.references.len())
    }
    ...
    pub fn parser(&self) -> Result<CellParser, TonCellError> {
        ...
        let mut bit_reader: BitReader<Cursor<&Vec<u8>>, BigEndian> = BitReader::endian(cursor, BigEndian);
        bit_reader.skip(self.start_bit as u32).map_cell_parser_error()?;

        Ok(CellParser { bit_len, bit_reader })
    }
    ...
    pub fn into_cell(&self) -> Result<Cell, TonCellError> {
        ...
        CellBuilder::new().store_bits(significant_bits, slice?.as_slice())?.build()
    }
    ...
    pub fn reference(&self, idx: usize) -> Result<&ArcCell, TonCellError> {
        ...
        self.cell.references.get(self.start_ref + idx).ok_or(TonCellError::InvalidIndex {
            idx,
            ref_count: self.end_ref - self.start_ref,
        })
    }
