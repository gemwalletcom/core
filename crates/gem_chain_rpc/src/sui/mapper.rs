...
use crate::sui::model::BalanceChange;
    ...
    pub fn map_transaction(transaction: SuiTransaction, block_number: i64) -> Option<Transaction> {
        ...
        if transaction.events.iter().any(|x| x.event_type.contains("Swap")) {
            ...
            let swap = match owner_balance_changes.len() {
                ...
                3 => {
                    let owner_balance_changes = owner_balance_changes
                        .iter()
                        .filter(|x| x.coin_type != SUI_COIN_TYPE)
                        .cloned()
                        .collect::<Vec<_>>();
                    Self::map_owner_balance_changes_swap_metadata(owner_balance_changes.clone(), fee.clone())?
                }
                _ => return None,
            };
