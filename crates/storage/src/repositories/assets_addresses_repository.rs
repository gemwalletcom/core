...
pub trait AssetsAddressesRepository {
    ...
    fn get_assets_by_addresses(&mut self, values: Vec<ChainAddress>, from_datetime: Option<NaiveDateTime>, prices_min_updated_at: Option<NaiveDateTime>) -> Result<Vec<AssetId>, DatabaseError>;
...
impl AssetsAddressesRepository for DatabaseClient {
    ...
    fn get_assets_by_addresses(&mut self, values: Vec<ChainAddress>, from_datetime: Option<NaiveDateTime>, prices_min_updated_at: Option<NaiveDateTime>) -> Result<Vec<AssetId>, DatabaseError> {
        Ok(AssetsAddressesStore::get_assets_by_addresses(self, values, from_datetime, prices_min_updated_at)?.asset_ids())
    }
