...
pub trait ParserStateRepository {
    ...
    fn set_parser_state_latest_block(&mut self, chain: &str, block: i64) -> Result<usize, Box<dyn Error + Send + Sync>>;
    fn set_parser_state_current_block(&mut self, chain: &str, block: i64) -> Result<usize, Box<dyn Error + Send + Sync>>;
}
