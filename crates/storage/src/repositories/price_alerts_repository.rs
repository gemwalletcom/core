...
pub trait PriceAlertsRepository {
    fn get_price_alerts(&mut self, after_notified_at: NaiveDateTime) -> Result<Vec<(primitives::PriceAlert, primitives::Price, primitives::Device)>, Box<dyn Error + Send + Sync>>;
...
impl PriceAlertsRepository for DatabaseClient {
    fn get_price_alerts(&mut self, after_notified_at: NaiveDateTime) -> Result<Vec<(primitives::PriceAlert, primitives::Price, primitives::Device)>, Box<dyn Error + Send + Sync>> {
        ...
        Ok(results.into_iter().map(|(alert, price, device)| {
            (alert.as_primitive(), price.as_price_primitive(), device.as_primitive())
        }).collect())
    }

    fn get_price_alerts_for_device_id(&mut self, device_id: &str) -> Result<Vec<primitives::price_alert::DevicePriceAlert>, Box<dyn Error + Send + Sync>> {
        let results = PriceAlertsStore::get_price_alerts_for_device_id(self, device_id)?;
        Ok(results.into_iter().map(|(alert, device)| primitives::price_alert::DevicePriceAlert {
