...
pub trait LinkTypesRepository {
    fn add_link_types(&mut self, values: Vec<LinkType>) -> Result<usize, DatabaseError>;
}

impl LinkTypesRepository for DatabaseClient {
    fn add_link_types(&mut self, values: Vec<LinkType>) -> Result<usize, DatabaseError> {
        let storage_values = values.into_iter().map(crate::models::LinkTypeRow::from_primitive).collect();
