...
pub fn map_currency_hex_to_symbol(currency: &str) -> Result<String, Box<dyn Error + Sync + Send>> {
    let currency_bytes = hex::decode(currency.trim_end_matches('0'))
        .map_err(|_| "Invalid currency hex")?;
    let symbol = String::from_utf8(currency_bytes.into_iter().filter(|&b| b != 0).collect())
        .unwrap_or_else(|_| currency.to_string());
    Ok(symbol)
}

pub fn map_account_object_to_asset(object: &AccountObject, token_id: String, chain: Chain) -> Result<Asset, Box<dyn Error + Sync + Send>> {
