...
use crate::models::{EarnPositionData, Yield, YieldDetailsRequest, YieldProvider, YieldTransaction};
...
impl Yielder {
    ...
    pub async fn positions(&self, provider: YieldProvider, request: &YieldDetailsRequest) -> Result<EarnPositionData, YieldError> {
        let provider = self.get_provider(provider)?;
