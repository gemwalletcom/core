...
#[cfg(test)]
mod tests {
    use super::{ETHEREUM_RECOVERY_ID_OFFSET, SecpSigningKey, apply_ethereum_recovery_id, sign_digest, sign_eth_digest};
    ...
    fn sign_digest_returns_raw_recovery_id() {
        ...
        let sig = Signature::try_from(rs.as_slice()).unwrap();
    ...
    fn sign_eth_digest_applies_offset() {
        let private_key = hex::decode(PRIVATE_KEY).unwrap();
        let (rs, v) = sign_digest(&DIGEST, &private_key).unwrap();
        let eth = sign_eth_digest(&DIGEST, &private_key).unwrap();

        assert_eq!(rs, &eth[..64]);
        assert_eq!(v + ETHEREUM_RECOVERY_ID_OFFSET, eth[64]);
    }
    ...
    fn apply_recovery_id_offset() {
        ...
        sig[64] = 0;
        apply_ethereum_recovery_id(&mut sig);
