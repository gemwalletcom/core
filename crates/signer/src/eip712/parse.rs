use crate::error::{SignerError, format_err};
...
pub fn parse_uint_value(value: Option<&Value>) -> Result<U256, SignerError> {
    match value {
        Some(Value::String(s)) => U256::from_str(s).map_err(format_err),
...
pub fn parse_int_value(value: Option<&Value>) -> Result<I256, SignerError> {
    match value {
        Some(Value::String(s)) => I256::from_str(s).map_err(format_err),
        Some(Value::Number(num)) => {
            if let Some(i) = num.as_i64() {
                return I256::try_from(i as i128).map_err(format_err);
            }
