use crate::model::{
    CoinQuery, CointListQuery, SearchTrending, SearchTrendingItemCoin, SimplePriceQuery,
};
...
impl CoinGeckoClient {
    ...
    pub async fn get_prices_by_ids(
        ...
        ids: Vec<String>,
        currency: &str,
    ) -> Result<SimplePrices, Box<dyn std::error::Error>> {
    ...
    pub async fn get_market_chart(&self, coin_id: &str) -> Result<MarketChart, Error> {
        let url = format!("{}/api/v3/coins/{}/market_chart?vs_currency=usd&days=max&interval=daily&precision=full",self.url, coin_id);
        let response = self
            .client
            .get(url)
            .send()
