...
impl AssetsClient {
    ...
    pub async fn get_tokenlist(&self, chain: &str) -> Result<TokenList, Box<dyn Error + Send + Sync>> {
        let url = format!("{}/tokenlists/{}.json", self.assets_url, chain);
        let response = self.client.get(&url).send().await?.json::<TokenList>().await?;
        Ok(response)
    }
    ...
    pub async fn get_asset_info(&self, asset_id: &str) -> Result<AssetInfo, Box<dyn Error>> {
        ...
        let url = if let Some(token_id) = id.token_id {
            format!("{}/blockchains/{}/assets/{}/info.json", self.assets_url, id.chain.as_ref(), token_id)
        } else {
            format!("{}/blockchains/{}/info/info.json", self.assets_url, id.chain.as_ref())
        };

        let response = self.client.get(&url).send().await?.json::<AssetInfo>().await?;
        Ok(response)
    }
}
